...
class App2 extends Component {
  ...
  getTrade = (area: string) => {
    fetch('http://13.125.55.96:8080/trade/search/area/' + area + '?no=0', {
      method: 'GET'
    })
      .then(res => res.json())
      .then(json => {
  ...
  componentDidMount() {
    console.log('did마운트 안', this.state.trade)
    ...
    //   }
    // }
    // End 클릭 이벤트 for Marker

    // 카카오에서 제공되는 라이브러리로 검색 결과를 불러와 마커를 표시하는 기능
    // var ps = new window.kakao.maps.services.Places();
    // ps.keywordSearch('삼성동',placesSearchCB);
    ...
    // 검색 결과를 통해 바운더리를 지정하고 맵에 마커 표시후 중앙점을 잡아주는 기능
    // let geocoder = new window.kakao.maps.services.Geocoder();
    ...
    //     }
    //     map.setBounds(bounds);
    //     let recenter = map.getCenter();
    //     console.log(recenter);
    //   }
    // };
    // geocoder.addressSearch('청담동', callback);
    ...
    function displayArea(area: any) {
      ...
      window.kakao.maps.event.addListener(polygon, 'click', (mouseEvent: any) => {
        ...
        for (let i = 0; i < trade.length; i++) {
          console.log(trade[i])
          if (trade[i].ttitle.length > 0) content += '   <div class="size"> <ul><li>게시글 : ' + trade[i].ttitle.substring(0, 10) + '...' + '<br/>가격 :' + trade[i].pprice + '</li></ul> </div>'
        }

...
public class Solution2 {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for (int t = 1; t <= T; t++) {
			...
			int[][] customers = new int[N][2];	// N명의 고객집 좌
			int[][] distance = new int[N+2][2]; // 회사좌표(0), N명의 고객좌표(순열로 순서 결정:1-N), 집좌표(N+1)
			p = new int[N]; 
	...
	// boolean : true => 다음 순열 생성 ok, false => 다음순열생성 불가(이미 제일 큰 순열)
	private static boolean nextPermutation() {
		//1. 뒤쪽부터 탐색하며 꼭대기(i) 찾기
		int i = N-1;
		while(i>0 && p[i-1]>=p[i]) --i;
		if(i==0) return false; // 이미 제일 큰 마지막 순열일므로 다음 순열 없음
		
		//2. 뒤쪽부터 탐색하며 교횐할 큰값(j) 찾기
		int j = N-1;
		while(p[i-1]>=p[j]) --j;
